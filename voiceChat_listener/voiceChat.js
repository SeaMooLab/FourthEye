...
export function setupVoiceChatListener(bot, targetGuild) {
    //CreateVC
    bot.on("text", (packet) => {
        switch (packet.type) {
            case "json_whisper": {
                ...
                if (obj.rawtext[0].text.includes(config.voiceChannelCommandPrefix + "createVoiceChannel")) {
                    ...
                    const category = guild.channels.cache.find((ch) => ch.type === ChannelType.GuildCategory && ch.name === config.voiceChannelsCategory);
                    ...
                    const fetchPromises = members.map((memberName) => fetchUserByUsername(memberName));
                    ...
                    Promise.allSettled(fetchPromises)
                        ...
                        const usernamesInPrivateChannels = [];
                        users.forEach((user) => {
        ...
        switch (packet.type) {
            case "json_whisper": {
                ...
                if (obj.rawtext[0].text.includes(config.voiceChannelCommandPrefix + "invite")) {
                    ...
                    const channelToBeInvitedTo = guild.channels.cache.find((ch) => ch.type === ChannelType.GuildVoice && ch.name === channelName);
                    //Get the Requesters User object in discord.
                    function fetchSingleMemberAndCheck(username) {
                        return new Promise((resolve, reject) => {
                            guild.members
                                .fetch({ query: username, limit: 1 })
                                .then((fetchedMembers) => {
                    ...
                    function fetchSingleMember(username) {
                        return new Promise((resolve, reject) => {
                            guild.members
                                .fetch({ query: username, limit: 1 })
                                .then((fetchedMembers) => {
